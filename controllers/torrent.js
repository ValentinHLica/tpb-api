...
module.exports = async (req, res, next) => {
  ...
  await axios.get(url).then((Torrent) => {
    ...
    $("#comments div").each((i, element) => {
      ...
      if ($(element).find('.byline img[title="VIP"]').attr("title") === "VIP") {
        commentOb.commentInfo.vip = true;
      }

      // Check if Uploader is VIP #2
      if (
        $(element).find('.byline img[title="Trusted"]').attr("title") === "VIP"
      ) {
